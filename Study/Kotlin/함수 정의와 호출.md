...
- Named Parameter와 Default Parameter의 사용법
- 최상위 함수와 최상위 프로퍼티의 동작 원리
- val과 const val의 차이
- 확장 함수와 확장 프로퍼티의 개념
- 확장 함수의 오버라이드 불가 원리 (정적 디스패치)

---

## 이름 붙은 인자(Named Parameter)
...
| **수신 객체** (Receiver Object) | 확장 함수가 호출되는 대상 값 | `"Kotlin"` |
...
| 구분 | 동적 디스패치 (Dynamic Dispatch) | 정적 디스패치 (Static Dispatch) |
|------|-----------------------------------|----------------------------------|
| **결정 시점** | 런타임 | 컴파일 타임 |
...
- Named Parameter: 인자 이름을 명시하여 순서 무관하게 전달, 이름 뒤에 위치 인자 불가
- Default Parameter: 파라미터 기본값 지정, 뒤에 선언하는 것을 권장
- 최상위 함수: 클래스 밖에 함수 선언 가능, 컴파일 시 파일명 기반 클래스로 변환
- const val: 컴파일 타임 상수, val: 런타임 값
- 확장 함수: 클래스 외부에서 멤버처럼 호출 가능한 함수, 정적 메서드로 컴파일
- 확장 함수 오버라이드 불가: 정적 디스패치 사용, 선언된 타입 기준으로 호출 결정
- 확장 프로퍼티: Backing Field 없이 getter/setter만 제공하는 문법적 장치
