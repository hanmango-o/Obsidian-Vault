...
- Kotlin의 예외 처리 기본 구조
- throw가 식(expression)인 이유
- try, catch, finally의 사용법
- Java와의 차이점 (체크 예외 구분 없음)
- try를 식으로 사용하는 방법
...
- throw는 식(expression): 다른 식 내부에 포함 가능
- try/catch/finally: Java와 동일한 구조
- 체크 예외 구분 없음: throws 절 불필요, 예외 처리 선택적
- try를 식으로 사용: 값을 변수에 대입 가능, 마지막 식이 결과값
