...
터치 이벤트는 **Activity → ViewGroup → View** 순서로 위에서 아래로 전달됩니다. 처리되지 않은 이벤트는 다시 위로 전파됩니다.
...
- Activity: 최초로 터치 이벤트를 수신하고 Window에 전달
- ViewGroup: 자식에게 전달할지, 자신이 처리할지 결정
- View: `onTouchListener` 또는 `onTouchEvent()` 호출
